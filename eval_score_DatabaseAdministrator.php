...
if(empty($userName_err)) {
    ...
    $skillSOM = $_POST['SkillSOM'];
    $skillOth = $_POST['SkillOth'];
    $internProject = $_POST['InternProject'];
    $internProjectOth = $_POST['InternProjectOth'];
    $course1 = $_POST['Course1'];
    $course2 = $_POST['Course2'];
    
    $jobType = "DataAdministrator";

    $skillScore = sizeof($skillSOM) * 43.00 + sizeof($skillOth) * 2.00;
    $courseScore = sizeof($course1) * 30.00 + sizeof($course2) * 5.00;
    ...
    $DatabaseAdministratorScore = $skillScore * 0.3 + $internProjectScore * 0.35 + $majorScore * 0.15 + $gpaScore * 0.1 + $schoolScore * 0.05 + $courseScore * 0.05;
    $sql3 = "SELECT userName, jobType FROM Score WHERE userName = '$userName' AND jobType = '$jobType'";
    $result3 = $mysqli->query($sql3);
    // $row3 = $result3->fetch_assoc();
    
    if (mysqli_num_rows($result3)) {
        $sqlUpdate = "UPDATE Score SET skillScore = '$skillScore', courseScore = '$courseScore', internProjectScore = '$internProjectScore', majorScore = '$majorScore', gpaScore = '$gpaScore', schoolScore = '$schoolScore', totalScore = '$DatabaseAdministratorScore' WHERE userName = '$userName' AND jobType = '$jobType'";
        if ($stmt = mysqli_prepare($mysqli, $sqlUpdate)) {
            ...
            $param_gpa = $gpaScore; 
            $param_total = $DatabaseAdministratorScore;
            ...
            $comb = array("jobType" => (string)$jobType,  
              ...
              "DatabaseAdministratorScore" => (string)$DatabaseAdministratorScore
              );
        ...
        // Bind variables to the prepared statement as parameters
            ...
            $param_totalScore = $DatabaseAdministratorScore;
