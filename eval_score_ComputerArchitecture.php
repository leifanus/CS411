...
if(empty($userName_err)) {
    ...
    $skillLWJCL = $_POST['SkillLWJCL'];
    
    $internProject = $_POST['InternProject'];
    $internProjectOth = $_POST['InternProjectDB'];
    
    $coursePCDE = $_POST['CoursePCDE'];
    
    $jobType = "ComputerArch";
    $skillScore = sizeof($skillLWJCL) * 20.00;
    $courseScore = sizeof($coursePCDE) * 25.00;
    ...
    $ComputerArchScore = $skillScore * 0.3 + $internProjectScore * 0.35 + $majorScore * 0.15 + $gpaScore * 0.1 + $schoolScore * 0.05 + $courseScore * 0.05;
    // echo("skill = ");
    // echo($skillScore);
    // echo(" intern = ");
    ...
    if (mysqli_num_rows($result3)) {
        $sqlUpdate = "UPDATE Score SET skillScore = '$skillScore', courseScore = '$courseScore', internProjectScore = '$internProjectScore', majorScore = '$majorScore', gpaScore = '$gpaScore', schoolScore = '$schoolScore', totalScore = '$ComputerArchScore' WHERE userName = '$userName' AND jobType = '$jobType'";
        if ($stmt = mysqli_prepare($mysqli, $sqlUpdate)) {
            ...
            $comb = array("jobType" => (string)$jobType,  
              ...
              "ComputerArchScore" => (string)$ComputerArchScore
              );
        ...
        // Bind variables to the prepared statement as parameters
            ...
            $param_totalScore = $ComputerArchScore;
